...
void translateShuriken(){
    ...
    float p2 [2] = {-0.4,-0.75};
    float p3 [2] = {0, -0.9};
    double* p =bezier(sBezier,p0,p1,p2,p3);
    sBezier+=0.002;
    shurikenX = p[0];
    shurikenZ = p[1];
}
...
void anim(){
    switch(trajectory){
        ...
        case SHURIKEN:{
            ...
            trajY = shurikenY;
            trajZ = shurikenZ;
            trajDirAngle = shurikenDirAngle;
            break;
        }
    }
    if(game_mode == REPLAY){
        //adjust camera position to follow trajectory
        xCamPos = trajX;
        yCamPos = trajY + 0.03;
        zCamPos = trajZ + 0.03;
//        
...
//        zCamDir = trajZ - 0.05;
    ...
    if(game_mode == SHOOT || game_mode == REPLAY){
        switch(trajectory){
            case BULLET:
            {
                translateBullet();
